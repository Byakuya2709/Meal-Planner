...
const enrichRecipeWithIngredientTypes = async (recipe) => {
  try {
    ...
    const allIngredientIds = parseIngredientsListFixed(
      recipe.ingredients_list_fixed
    );
    ...
    const requiredIds = riError
      ? []
      : (recipeIngredientsData || []).map((ri) => ri.ingredient_id);
...
export const supabaseRecipeService = {
  ...
  async findRecipe(selectedIngredientIds) {
    try {
      ...
      const { data: selectedIngredients, error: ingredientsError } =
        await supabase
          .from("ingredients")
          .select("name")
      ...
      const { data: matchedRecipes, error: searchError } = await supabase.rpc(
  ...
  async findRecipesByIngredients(selectedIngredientIds) {
    try {
      ...
      log(
        ...
        selectedIngredients.map((i) => ({ id: i.id, name: i.name }))
      );
      ...
      allRecipeIngredients.forEach((ri) => {
        ...
        if (selectedIngredientIds.includes(ri.ingredient_id)) {
          recipeMatchMap[ri.recipe_id].matchedIngredients.push(
            ri.ingredient_id
          );
        }
      });
      ...
      log(
        `Found ${matchedRecipeIds.length} recipes matching in recipe_ingredients (required only)`
      );
      ...
      if (matchedRecipeIds.length === 0) {
        log(
          "⚠️  No matches in recipe_ingredients (required), fallback to ingredients_list_fixed..."
        );
        ...
        allRecipes.forEach((recipe) => {
          ...
          const matchedIds = allIngredientIds.filter((id) =>
        ...
        log(
          `✅ Found ${matchedRecipeIds.length} recipes from ingredients_list_fixed fallback`
        );
      }
      ...
      if (matchedRecipeIds.length === 0) {
        ...
        return {
          ...
          data: await Promise.all(
            (fallbackRecipes || []).map(async (recipe) => {
      ...
      const enrichedRecipes = await Promise.all(
        recipes.map(async (recipe) => {
          ...
          const matchScore = Math.round(
            (matchRatio * 60 + coverageRatio * 25 + simplicityScore * 15) *
              fallbackPenalty
          );
      ...
      log(
        ...
        topRecipes.map((r) => ({
          ...
          secondaryIngredients:
            r.secondaryIngredients?.map((i) => i.name) || [],
        }))
      );
  ...
  /**
   * Ghi nhận user đã chọn recipe để nấu
   * Trigger sẽ tự động cập nhật impact_stats
   */
  async recordRecipeSelection(recipeId, selectedIngredientIds) {
    try {
      const {
        data: { user },
      } = await supabase.auth.getUser();
      ...
      if (!user) {
        log("⚠️ User not authenticated, skipping selection record");
        return { success: true };
      }

      log("Recording recipe selection:", { recipeId, userId: user.id });
      ...
      // const ingredientNames = selectedIngredientIds?.map((i) => i.name) || [];

      // Insert vào user_recipe_selections
      // Trigger sẽ tự động cập nhật impact_stats
      const { error } = await supabase.from("user_recipe_selections").insert({
        user_id: user.id,
        recipe_id: recipeId,
        selected_ingredients: selectedIngredientIds || [],
      });

      if (error) throw error;

      log("✅ Recipe selection recorded, impact stats updated via trigger");

      return { success: true };
    } catch (error) {
      return handleError("recordRecipeSelection", error);
    }
  },
};
